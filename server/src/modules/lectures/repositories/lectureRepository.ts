...
export class LectureRepository implements ILectureRepository{
  ...
  async updateCourseStatus(courseId: mongoose.Types.ObjectId, status: string): Promise<ICourse | null> {
    return await Course.findByIdAndUpdate(courseId, { isApproved: status }, { new: true });
  }
