...
const ForgotPasswordModal = ({open, handleClose}:{open: boolean, handleClose:()=>void}) => {
  ...
  const handleSubmit = async() => {
    try {
        ...
        const errorMsg = (error instanceof AxiosError && error.response?.data?.message) || "An unexpected errror occured"
        ...
        >
          <Box sx={{ border: "none", outline: "none" }}>
            ...
            <Typography variant="body2" sx={{ mb: 2 }}>
              Enter your registered email address to recieve a password reset
              link{" "}
            </Typography>
            <TextField
              fullWidth
